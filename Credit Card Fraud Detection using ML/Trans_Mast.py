...
State_Name = {"Gujarat": 0, "Hyderabad": 1,
              "Karnataka": 2, "Maharashtra": 3, "Punjab": 4}
...
def chk_Amt(CNo, Ent_Bal):
    ...
    Sqlstr = "Select balance,status From Card_master WHERE Cardno='{0}'".format(
        CNo)
...
def SaveDetails():
    ...
    if proc_Mode:
        ...
        Val_set = [int(id_V[0]), int(Shop_CodeNam[TMMNameEL.get()]), int(
            State_Name[TMStateEL.get()]), int(Trans_AmtEL.get())]
...
#        print(fake_not)
        ...
        if fake_not == 1:
            ...
            for element in transData:
                ...
                c.execute(
                    "INSERT INTO Tamp_master VALUES (?,?,?,?,?,?,?,?,?)", element)

            time.sleep(16)

            result_lblinfo("Transaction Successfully done!! AS Genuine ENTRY<<======")
            # import messagebox 
            # messagebox.showinfo("Congratulations", "\n Transaction Successfully done!! AS Genuine ENTRY")
            ...
            result_lblinfo("Transaction Failed!! AS FRAUD ENTRY<<======")
            # import messagebox 
            # messagebox.showinfo("Failed", "\n /n Transaction Failed!! AS FAKE ENTRY  <<======")
...
TMMNameEL = ttk.Combobox(ttop, values=list(
    Shop_CodeNam.keys()), width=40, textvariable=TMMName)
TMMNameEL.state(['readonly'])
# lambda event: print(Shop_CodeNam.keys()))
TMMNameEL.bind("<<ComboboxSelected>>", Cmbo1_sel)
TMMNameEL.current(0)


TMStateEL = ttk.Combobox(ttop, values=list(
    State_Name.keys()), width=40, textvariable=TMState)
TMStateEL.state(['readonly'])
TMStateEL.bind("<<ComboboxSelected>>",
               lambda event: print(State_Name[TMStateEL.get()]))

TMStateEL.current(0)


# Set up labels
TMCodeLB = tk.Label(ttop, text='Transection Code : ',
                    bg='khaki2')  # More labels =""
# TMMCodeLB = tk.Label(ttop, text='Merchant Code : ',bg='khaki2')  # More labels=""
TMMNameLB = tk.Label(ttop, text='Merchant Name : ',
                     bg='khaki2')  # More labels=""
TMStateLB = tk.Label(ttop, text='State : ', bg='khaki2')  # More labels=""
TMMSiteLB = tk.Label(ttop, text='Address\Site Name: ',
                     bg='khaki2')  # More labels=""
Trans_AmtLB = tk.Label(ttop, text='Purchase Amount : ',
                       bg='khaki2')  # More labels=0
DCoLB0 = tk.Label(ttop, text='----------------', fg='brown',
                  bg='khaki2', font=('times', 18, ' bold '))  # ^
DCoLB1 = tk.Label(ttop, text='--------------------------------------------------------------------------------------',
                  fg='brown', bg='khaki2', font=('times', 18, ' bold '))  # ^

TCCNoLB = tk.Label(ttop, text='Credit Card No : ',
                   bg='khaki2')  # More labels""
TCCPinLB = tk.Label(ttop, text='PIN : ', bg='khaki2')  # More labels=""

DCoLB2 = tk.Label(ttop, text='----------------', fg='brown',
                  bg='khaki2', font=('times', 18, ' bold '))  # ^
DCoLB3 = tk.Label(ttop, text='--------------------------------------------------------------------------------------',
                  fg='brown', bg='khaki2', font=('times', 18, ' bold '))  # ^

ClrB = tk.Button(ttop, text='Clear', bg='cyan3',
                 fg='red', command=Clear_entry, width=10)
ProcB = tk.Button(ttop, text='Process Details', bg='cyan3',
                  fg='red', command=proc_detail, width=15)

DCoLB4 = tk.Label(ttop, text='----------------', fg='brown',
                  bg='khaki2', font=('times', 18, ' bold '))  # ^
DCoLB5 = tk.Label(ttop, text='--------------------------------------------------------------------------------------',
                  fg='brown', bg='khaki2', font=('times', 18, ' bold '))  # ^


ProcInfoLB = tk.Label(ttop, text='----------------------Information-----------------------',
                      fg='brown', bg='khaki2', font=('times', 15, ' bold '))  # ^


TM_StatusLB = tk.Label(ttop, text='Status : ', bg='khaki2')  # More labels=""


TMInfoLB = tk.Label(ttop, text='------------Result---------------',
                    fg='brown', bg='khaki2', font=('times', 15, ' bold '))  # ^
...
BuyB = tk.Button(ttop, text='Purchase', bg='cyan3',
                 fg='red', command=SaveDetails, width=15)
BuyB.grid(column=0, row=17, columnspan=1, padx=5, sticky=tk.W)
